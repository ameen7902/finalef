...
async def submit_tiebreaker_result(update: Update, context: ContextTypes.DEFAULT_TYPE):
    # ... (all your existing code, including saving states) ...

    save_state("fixtures", fixtures_data)
    save_state("tournament_state", tournament_state)

    # --- ADD THESE DEBUG PRINTS ---
    print(f"DEBUG: Attempting to send confirmation reply to user {update.effective_user.id}...")
    try:
        await update.message.reply_text(
        ...
        print("DEBUG: Confirmation reply sent to user.")
    except Exception as e:
        print(f"ERROR: Failed to send reply to user: {e}")
        # Optionally, notify admin if reply fails:
        # await context.bot.send_message(ADMIN_ID, f"Error sending reply to user after tiebreaker: {e}")


    print(f"DEBUG: Attempting to send group message to GROUP_ID: {GROUP_ID}...")
    try:
        await context.bot.send_message(
            GROUP_ID, 
            f"*🎉 Tiebreaker Result: Group {escape_markdown_v2(group_name)} Completed! 🎉*\n\n"
            f"The tiebreaker match between {get_player_display_name(players.get(tied_player1_id, {}))} and {get_player_display_name(players.get(tied_player2_id, {}))} has concluded.\n\n"
            f"Qualifier: *{get_player_display_name(players.get(winner_id, {}))}*\n"
            f"Eliminated: *{get_player_display_name(players.get(loser_id, {}))}*\n\n"
            f"Congratulations to {get_player_display_name(players.get(winner_id, {}))} for advancing to the Knockout Stage!",
            parse_mode=ParseMode.MARKDOWN_V2
        )
        print("DEBUG: Group message sent.")
    except Exception as e:
        print(f"ERROR: Failed to send message to group chat: {e}")
        # Send a message to admin if group message fails
        await context.bot.send_message(ADMIN_ID, f"Error sending tiebreaker result to group chat: {e}")
